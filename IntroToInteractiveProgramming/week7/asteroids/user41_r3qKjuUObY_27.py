...
class Sprite:
    ...
    def update(self):
        self.angle += self.angle_vel
        self.pos[0] = (self.pos[0] + self.vel[0]) % WIDTH
        self.pos[1] = (self.pos[0] + self.vel[0]) % HEIGHT        
...
def rock_spawner():
    global a_rock
    a_rock = Sprite([pos_or_neg(random.randrange(0, 800)), pos_or_neg(random.randrange(0, 600))],
                [pos_or_neg(random.randrange(1, 4)), pos_or_neg(random.randrange(1, 4))],
                0, pos_or_neg(random.randrange(1, 10)/100.0),asteroid_image, asteroid_info)
    print a_rock.vel
