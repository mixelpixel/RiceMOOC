#- "ASTEROIDS", pdk, started: 5/16/16 @ 2100, finished 5/21/16 @ 0300
#---------------------------------------------------------------------72
...
def angle_to_vector(ang):
    ...
    converts the angle to a vector: REVIEW TRIGONOMETRY'''
...
class Ship:
    ...
    def torq(self, spin):
        ...
        if spin == 'cw': self.angle_vel += self.angle_spin
        elif spin == 'ccw': self.angle_vel -= self.angle_spin
...
def key_press(press):
    global my_ship
    if press == simplegui.KEY_MAP['left']: #IS IT BETTER TO MAKE AN IF/ELIF/ELIF... CHAIN?
        my_ship.torq('ccw')
...
def key_release(release):
    global my_ship
    if release == simplegui.KEY_MAP['left']: #IS IT BETTER TO MAKE AN IF/ELIF/ELIF... CHAIN?
        my_ship.torq('cw')
