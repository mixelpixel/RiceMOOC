...
NUM_LIVES = 5 #Starting number
SAFETY = 50 #For distance calculation when spawning asteroids
time, score, lives = 0, 0, NUM_LIVES
started = False
...
num_of_missiles = 5 # more with higher score?
missile_lifespan = 90
...
#            print len(missile_group), [missile.__str__() for missile in missile_group]
...
#        self.age +=1 # THIS SEEMS TO LENGTHEN THE LIFESPAN, WHY?
        # ALSO, I DON'T NEED ASTEROIDS TO HAVE AN AGE
...
def draw(canvas):
    ...
    global time, score, started, lives, rock_group, missile_group
...
# HANDLING THE MISSILE AGE HERE ALSO SLOWS DOWN/LENGTHENS THE LIFESPAN, WHY?
# AND THIS WHETHER AGE+1 IS HANDLED IN SPRITE CLASS OR HERE, WHY?
#    for missile in list(missile_group):
##        missile.age +=1
#        print missile.age
...
def key_press(press):
    ...
    if started:#BETTER TO MAKE AN IF/ELIF/ELIF OR IF/IF/IF... CHAIN?
...
def draw_lives(canvas):
    '''draws the lives as mini-ship facing up
    like the original Asteroids'''
#    [canvas.draw_image(my_ship.image, my_ship.image_center, my_ship.image_size,
