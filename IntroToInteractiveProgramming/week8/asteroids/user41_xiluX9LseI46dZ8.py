#- "ROCK STAR" by pdk
#- 160524@~2000start; 160528@2200finished
#- For Coursera: Rice U. Intro to Interactive Programming, pt.2
#- Based on the classic arcade game, "Asteroids"
#---#----1----#----2----#----3----#----4----#----5----#----6----#----7-2--#----8----#----91234567100
# TO DO: MAKE SHIP EXPLODE WHEN IT HITS A ROCK
# RESTART SHIP IN CENTER AFTER HIT AND MAKE SURE NOT ON TOP OF A ROCK
# "FLASH IN" ROCK RESPAWNING?
# ADD LEVELS; FIRING FROM ALL FIVE CANNONS, ETC.
# ANIMATE FINAL DEATH? MAYBE CHANGE SHIP IMAGE SO IT DEGRADES WITH EACH LIFE LOST?
# HEALTH BAR?
# MAKE AN ALIEN SHIP FLY BY AND SHOOT AT SHIP!!!!!!
# REWORK OOP, EXTRACT "MY_SHIP" CALLS
# HMM... RESTATING GAME DOESN't RESET DIFFICULTY LEVEL BASED ON SCORE, WHY????

import simplegui, math, random
#I'll need to remove all the instances of "math." and "random."
#but I'd like to try this way for PyGame:
#from math import sin, cos, sqrt, pi
#from random import random, choice, randrange
#...and see if there is an improvement in performance or less memory used

# globals bindings - GET RID OF ALL THIS CRAP FOR PYGAME AND UPLOAD TO GIT
def declare_globals():
    '''I know... "globals"'''
    global WIDTH, HEIGHT, SCREEN_SIZE, NUM_LIVES, SAFETY
    global time, score, lives, started
    global ship_radius, rockets_volume, FRICTION, SPIN
    global rock_group, num_of_asteroids, asteroid_radius
    global missile_group, num_of_missiles, missile_lifespan
    global msl_thrust, missile_radius, missile_volume
    global explosion_group
    global soundtrack_volume
    ...
    num_of_asteroids = 4 #Increase with score? maybe timer?
...
def image_and_sound_declarations():
    ...
    global explosion_info, explosion_image
    global soundtrack, missile_sound, ship_thrust_sound, explosion_sound

    splash_info = ImageInfo([200, 150], [400, 300])
    splash_image = simplegui.load_image("https://dl.dropbox.com/s/gc6zio2l0ye5afv/my_splash.png")
    ...
    ### MADE w/GIMP 2.8 & thanks to https://gimper.net/resources/sprite-sheet-animation-tiles.633/
    explosion_info = ImageInfo([64, 64], [128, 128], 17, 24, True)
    explosion_image = simplegui.load_image("https://dl.dropbox.com/s/f5g210fvf0tx83t/explosion_sheet.png")
    ...
    ship_thrust_sound = simplegui.load_sound("https://dl.dropbox.com/s/7hwlx7xfcfab4ni/thrust.mp3")
    explosion_sound = simplegui.load_sound("https://dl.dropbox.com/s/r9kzfdq45lwr24j/explosion.mp3")
...
def angle_to_vector(ang):
    ...
    converts the radian angle to a vector: thank you, TRIGONOMETRY.
...
def pos_or_neg(num):
    ...
    #it nicely avoids zero values & CodeSkulptor does not support random.uniform
...
def initial_ship_orientation():
    '''One of four orientations, up, down, left or right'''
...
def group_collide(group, other_object):
    ...
    for sprite in list(group):
        if sprite.collision(other_object):
            # NEXT LINE CREATES EXPLOSION MATCHING THE ASTEROIDS VELOCITY AND SUCH :)
            explosion_group.add(Sprite(sprite.pos, sprite.vel, sprite.angle, sprite.angle_vel, explosion_image, explosion_info, explosion_sound))
            group.remove(sprite)
            return True
...
class Ship:
    ...
    def __init__(self, pos, vel, angle, image, info):
        ...
        self.angle_spin = SPIN #change in angular velocity
        self.image = image
...
class Sprite:
    ...
    def __init__(self, pos, vel, ang, ang_vel, image, info, sound = None):
        ...
        self.age = 0
        if sound: #WHAT IF I NEEDED EXPLOSIONS TO HAVE A DIFFERENT LEVEL?
            sound.rewind()
            sound.play()
            sound.set_volume(missile_volume)

    def __str__(self):
        '''mostly just used for test/build'''
        if self.image == asteroid_image:
            return 'An asteroid'
        elif self.image == missile_image:
            return 'A missile'
        elif self.image == explosion_image:
            return 'An explosion'
        else:
            return 'A sprite'
    ...
    def draw(self, canvas):
        ...
        if self.animated:
            anim_sheet_center = [self.image_center[0] + (self.age * self.image_size[1]), self.image_center[1]]
            canvas.draw_image(self.image, anim_sheet_center, self.image_size,
                              self.pos, self.image_size, self.angle)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,
                              self.pos, self.image_size, self.angle)
...
def rock_spawner():
    ...
    global rock_group, SAFETY, num_of_asteroids
    # THIS NEEDS WORK
    min_val = 1
    max_vel = 3
    if score > 500:
    ...
    vel = [pos_or_neg(random.random() * random.randrange(min_val, max_vel)),
           pos_or_neg(random.random() * random.randrange(min_val, max_vel))] #start slower & increase as game progresses?
    ...
    dist(position, my_ship.pos) >  rock_diameter + ship_diameter + SAFETY:
        rock_group.add(Sprite(position, vel, ang, ang_vel, #LOCAL VARIABLES
                              asteroid_image, asteroid_info))
    # ONCE A TIMER-SECOND, THE NEXT LINE REMOVES ALL THE EXPLOSION GROUP SPRITES SO THEY DON'T HOG MEMORY
    [explosion_group.remove(explode_anim) for explode_anim in list(explosion_group) if explode_anim.age > 25]
...
def draw(canvas):
    ...
    # update lives and reset when lives == 0
    if group_collide(rock_group, my_ship):
        ...
        if lives == 0:
            ...
            score = 0
    # update score when an asteroid is hit by a missile
...
def click(pos):
    '''Mouse click handler to start game from splash screen'''
...
def draw_lives(canvas):
    ...
    so the "lives' ships don't also fire their rockets'''
