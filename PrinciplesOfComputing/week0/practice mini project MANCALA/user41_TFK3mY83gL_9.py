...
#            self._game.append(config)
    ...
    def __str__(self):
        ...
        """
#        store_on_right = list(self._game)
#        store_on_right.reverse()
#        return str(store_on_right)
        store_on_right_end = self._game[::-1]
        return str(store_on_right_end)
    ...
    def plan_moves(self):
        ...
        print 'MOVE THESE HOUSE NUMBERS IN THE LIST ORDER FROM LEFT TO RIGHT'
        return list_of_moves

# Create tests to check the correctness of your code
def test_suite():
    config = [0, 1, 1, 3, 0, 0, 0]
    print 'SUBMITTING TO SolitaireMancala('+ str(config)\
            +')\nSTORE is on the LEFT', config
    x = SolitaireMancala()
    print 'initialize board', x
    x.set_board(config)
    print 'set board for counter-clockwise play\ni.e Left > Right', x
    ...
    print 'set board', x.__str__()
    print
    print 'THE FOLLOWING PRINT STATEMENTS ARE WITHIN THE\n\
    PLAN_MOVES METHOD\n',x.plan_moves()
